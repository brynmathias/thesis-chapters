...
The \ac{susy} model considered is the \ac{cmssm}\mcite{ref:CMSSM} which is 
...
results shown by previous and contemporary experiments \mcite{RA1Paper2011, 
...
\subsection{Signal Efficiency for the Background Selection} % (fold)
...
FastSim\mcite{1742-6596-219-3-032053} rather than \ac{cms} 
FullSim\mcite{1742-6596-331-3-032015} the acceptance differs between the signal 
...
\texttt{PDF4LHC}\mcite{pdf4lhc} working group recommendations. On the event 
...
considered \ac{pdf} sets (CTEQ6.1\mcite{cteq6.1}, MSTW2008nlo68cl\mcite{MSTW} and 
NNPDF2.0\mcite{NNPDF}) and for the variations of each of their errors. The 
...
\begin{figure}[!h]
  \begin{center}
    ...
    efficiency in the high efficiency bins is of the order of $10\%$ for all 
    points.}
...
\section{Effect of Jet Energy Scale Variations on Signal Efficiency} 
% (fold)
...
their uncertainties, which are derived in \mcite{Chatrchyan:2011ds}. These 
...
\texttt{SoftSUSY}\mcite{Allanach:2001kg}, the signal events are generated at 
leading order using \texttt{Pythia 6.4}\mcite{pythia}, the \ac{nlonll} process 
dependent  cross sections are calculated using 
\texttt{PROSPINO}\mcite{Beenakker:1996ch} using the \texttt{CTEQ6}\mcite{cteq6.1} 
...
calculated using \ac{nlonll} cross sections using CL$_s$\mcite{cls-pdg}, the 
...
The observed yields are also used to set limits on \ac{sms} models. This allows 
...
Figure~\ref{fig:limits-sms} shows the observed limits at 95$\%$ confidence level obtained using CL$_{s}$ as a function of $m_{\Psquark(\PSgluino)}$ and $m_{LSP}$. The solid black lines represent the observed median limit assuming \ac{nlonll}\mcite{Beenakker:1996ch,susy-nlo-nll} \ac{susy} cross sections for squark pair production in the limit of de-coupled gluinos and vice versa for the gluino production models. The thin black lines represent the limit when this cross section is varied by $\pm1\sigma$ of the theoretical uncertainty, the dashed purple lines represent the expected limit and its $\pm1\sigma$ uncertainty.
