...
To obtain the systematic uncertainties a set of closure tests were preformed 
with data in order to identify any possible sources of systematic uncertainty 
in the background prediction method. The method uses the same translation 
factor method as the background prediction (see 
Equation~\eqref{eq:traslationfactor}), however the translation factors are 
recomputed to predict between control samples. The agreement between the 
prediction and the observed number of events is expressed as 
$(N_{obs}-N_{pred})/N_{pred}$ per \HT bin. No systematic uncertainty is 
assigned to the level of agreement, thus the level of closure is defined by the 
statistical significance of the sample, in this way the closure tests give a 
\HT dependant measure of systematic biases in the prediction method.

The closure tests described in this section measure the closure after an \alt 
cut, between samples where there is an \alt cut applied and those where there 
is not an \alt requirement as well as predicting between the \Pbottom 
multiplicity bins. As described in 
Section~\ref{sec:extension_to_higher_analysis_dimensions_} the Montecarlo 
simulation of the \Pbottom-Jet reconstruction is corrected to the data measured 
values for tag and miss-tag rate. This allows the choice of a systematic 
uncertainty stemming from improper simulation of the background processes, 
detector in efficiencies and kinematic effects not considered in the Montecarlo 
generation as well as effects from the theoretical uncertainties on the 
relative production and decay cross sections of each of the considered 
processes.


\paragraph{Control samples with out an \alt cut.} % (fold)
\label{par:control_samples_with_out_an_alt_cut_}
It is to be noted that for the closure tests between the samples that do not require an \alt cut and those that do that due to the change in trigger thresholds throughout the run that it is only possible to perform the closure tests over the full \HT range on \unit{3.9}{\invfb} of data, during which the \HT leg of the \muht trigger is not higher then \unit{200}{\GeV}.
% paragraph control_samples_with_out_an_alt_cut_ (end)
