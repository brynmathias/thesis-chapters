...
than 16. The events studied are taken with a single Mu HLT trigger, this removes
any biases on the turn on from pre selecting with a hadronic trigger.
To test the performance of the trigger system under high pile up conditions 
($<PU> \leq 32$) a dedicated high pile up fill of the LHC was performed.
The high pile up fill is studied from two points of view, the first is with
regard to standard AK5 calorimeter jets. Secondly the performance is measured
against Ak5 calorimeter jets that are corrected for pile up.
...
\begin{equation}
    fit = \frac{\epsilon}{2} \times (1 + \frac{2}{\sqrt{\pi}}\int^{x}_{0}e^{-t^{2}}dt)/\sqrt{2}\sigma. ~ where~ t = \epsilon - \mu
\label{eqn:erf}
\end{equation}
...
\end{tabular}
\caption{Results of an error function fit to the turn on curves for \Lone 
single jet triggers in the run 2011B high pile up fill. Preselected on an 
