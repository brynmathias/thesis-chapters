\chapter{Reconstruction and Event selction} % (fold)
...
dedicated modeling of the electron energy loss and fitted with a Gaussian Sum 
...
\label{sub:photons}
Photons are reconstructed in the ECAL using the same methods as used to reconstruct elections. However an anti selection is performed on the tracking requirements. For photons which do not convert in the tracker a supercluster energy deposit is required with no associated electron track, for photons which undergo pair production in the tracker material the tracks associated to the super clusters do not originate from the interaction vertex, instead they appear in a layer of the tracker. For unconverted photons the ratio of the energy deposited in the central ECAL crystal to the energy of the surrounding  $3\times3$ group of ECAL crystals is used as an identification variable. Signal events are vetoed if they contain a photon with \ET $>$ \unit{25}{\GeV}, 
